...
const sharingtextarea = document.getElementById("sharing-doctor-message-input");
...
function ShareData(medixID, message){
    const login_endpoint = "PHP Modules/Sharing.php";
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && xhr.status === 200) {
        ...
        if (response.success) {
          ...
          sharingloading_indicator.classList.add("hide");
            if(response.reason){
                sharingerror_messages[1].textContent = "Your data is already shared.";
            }
            else{
    ...
    xhr.send("medixid=" + encodeURIComponent(medixID) + "&message=" + encodeURIComponent(message));
  }
  ...
  async function validate_doctor(input, index) {
   ...
   function doctor_check(medixid) {
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.open("POST", "PHP Modules/DoctorCheck.php");
      xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    sharingtextarea.addEventListener('focus', function(){
      sharinglabels[1].classList.add("active");
    })
    sharingtextarea.addEventListener('blur', function(){
      if(sharingtextarea.value === ""){
      sharinglabels[1].classList.remove("active");
      }
    })
    sharingForm.addEventListener("submit", function (event) {  
      ...
      if (doc_status === "true") {
          ...
          ShareData(doc_id,document.getElementById('sharing-doctor-message-input').value );
      }
    });
  });